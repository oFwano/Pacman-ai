...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            hitsWall = self.walls[nextx][nexty]
            if not hitsWall:
                    new_expl = explored_corners
                    if (nextx,nexty) in self.corners  and (nextx,nexty) not in explored_corners:
                        new_expl = new_expl + [(nextx,nexty)]
                    successors.append((((nextx, nexty), new_expl), action, 1))
        self._expanded += 1 # DO NOT CHANGE
        return successors
...
def cornersHeuristic(state, problem):
    ...
    while len(nexplored) != 0:
        minMD = min([(util.manhattanDistance(node,c),c) for c in nexplored])
        h = h + minMD[0]
        node = minMD[1]
        nexplored.remove(minMD[1])
    return h
